                    ...
                    <div id="memberpages">
                        <div id="memberinfo">
                            <table>
                                <tr>
                                    <th scope="row">
                                        <?php _e('member number', 'usces') ?>
                                    </th>
                                    <td class="num">
                                        <?php esc_html_e($usces_members['ID']) ?>
                                    </td>
                                    <th>
                                        <?php _e('Strated date', 'usces') ?>
                                    </th>
                                    <td>
                                        <?php esc_html_e( mysql2date(__('Y/m/d'), $usces_members['registered']) ) ?>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">
                                        <?php _e('Full name', 'usces') ?>
                                    </th>
                                    <td>
                                        <?php usces_localized_name( $usces_members['name1'], $usces_members['name2'] ) ?>
                                    </td>
                                    <?php if(usces_is_membersystem_point()) : ?>
                                        <th>
                                            <?php _e('The current point', 'usces') ?>
                                        </th>
                                        <td class="num">
                                            <?php esc_html_e($usces_members['point']) ?>
                                        </td>
                                        <?php else : ?>
                                            <th class="space">&nbsp;</th>
                                            <td class="space">&nbsp;</td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php esc_html_e($usces_members['mailaddress1']) ?>
                                    </td>
                            ...
                            <ul class="member_submenu">
                                <li class="edit_member">
                                    <a href="<?php echo USCES_MEMBER_URL ?>">
                                        <?php _e('会員ページトップへ戻る', 'usces') ?>
                                    </a>
                                </li>
                            ...
                            <h3><?php _e('ギフト用配送先を登録', 'usces') ?></h3>
                            <div class="msa_area">
                                <div class="msa_total">
                                    <?php _e('登録件数', 'usces') ?><span id="msa_num"></span>件
                                        <input id="new_destination" type="button" value="<?php _e('新しい配送先を追加する', 'usces') ?>" />
                                </div>
                                <?php if( isset($_GET['msa_backurl']) ) : $cusurl = str_replace( 'customerinfo=1','customerinfo=msa', USCES_CUSTOMER_URL ); ?>
                                    ...
                                    <div class="allocation_dialog_exp">１．「新しい配送先を登録する」を押すと、新規登録画面になります。
                                        <br>２．お名前やご住所などの入力が終わりましたら「登録」を押して下さい。
                                        <br>３．登録がすべて終わりましたら、画面下の「発送先設定に戻る」を押して下さい。
                                        <br>
                                        <br>※ （ご本人）と書かれた配送先は削除できません。登録した配送先を変更・削除する場合は「編集する配送先を選ぶ」から（ご本人）以外のデータを選択して下さい。</div>
                                    <?php else: ?>
                                        <div class="allocation_dialog_exp">１．「新しい配送先を登録する」を押すと、新規登録画面になります。
                                            <br>２．お名前やご住所などの入力が終わりましたら「登録」を押して下さい。
                                            <br>
                                        ...
                                        <?php endif; ?>
                                            <div class="msa_operation">
                                                <label for="destination" class="destination_label">編集する配送先を選ぶ</label>
                                                <select id="destination"></select>
                                                <?php do_action('msa_action_memberinfo_page_operation'); ?>
                                            </div>
                                            <div class="msa_field_block">
                                                <div class="msa_title">
                                                    <?php _e('配送先', 'usces') ?> : <span id="destination_title" class="msa_title_inner"></span></div>
                                                <div class="msa_field">
                                                    <label for="msa_company">
                                                        <?php _e('法人名', 'usces') ?>
                                                    </label>
                                                ...
                                                <div class="msa_field">
                                                    <label for="msa_furigana">
                                                        <?php _e('フリガナ', 'usces') ?>
                                                    </label>姓
                                                    <input id="msa_furigana" name="msa_furigana" type="text" />名
                                                ...
                                                <div class="msa_field">
                                                    ...
                                                    <?php if(isset($usces->options['address_search']) && 'activate' == $usces->options['address_search']){ echo "<input type='button' id='search_zipcode' class='search-zipcode button' value='住所検索' onclick=\"AjaxZip3.zip2addr('msa_zip', '', 'msa_pref', 'msa_address1');\">"; } ?><span id="zip_message" class="msa_message"></span></div>
                                                <div class="msa_field">
                                                    <label for="msa_pref">
                                                ...
                                                <div class="msa_field">
                                                    ...
                                                    <input id="msa_fax" name="msa_fax" type="text" /><span id="fax_message" class="msa_message"></span></div>
                                                <div class="msa_field">
                                                    <label for="msa_note">
                                                        <?php _e('備考', 'usces') ?>
                                                    </label>
