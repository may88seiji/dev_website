...
/**********************************************************************/
	...
	define('USCES_COL_ITEM_DIVISION', 20);//DLSELLER
	define('USCES_COL_ITEM_CHARGING_TYPE', 21);//DLSELLER
	define('USCES_COL_ITEM_SHIPPING', 22);	//10
	define('USCES_COL_ITEM_DELIVERYMETHOD', 23);	//11
	define('USCES_COL_ITEM_SHIPPINGCHARGE', 24);	//12
	define('USCES_COL_ITEM_INDIVIDUALSCHARGE', 25);	//13

	define('USCES_COL_ITEM_FREQUENCY', 26);//DLSELLER
	define('USCES_COL_ITEM_CHARGINGDAY', 27);//DLSELLER
	define('USCES_COL_DLSELLER_INTERVAL', 28);//DLSELLER @
	define('USCES_COL_DLSELLER_VALIDITY', 29);//DLSELLER @
	define('USCES_COL_DLSELLER_FILE', 30);//DLSELLER @
	define('USCES_COL_DLSELLER_DATE', 31);//DLSELLER @
	define('USCES_COL_DLSELLER_VERSION', 32);//DLSELLER @
	define('USCES_COL_DLSELLER_AUTHOR', 33);//DLSELLER @
	define('USCES_COL_DLSELLER_PURCHASES', 34);//DLSELLER
	define('USCES_COL_DLSELLER_DOWNLOADS', 35);//DLSELLER

	define('USCES_COL_CATEGORY', 36);	//19
	define('USCES_COL_POST_TAG', 37);	//20
	define('USCES_COL_CUSTOM_FIELD', 38);	//new

	$add_field_num = apply_filters( 'dlseller_filter_uploadcsv_item_field_num', 0 );

	define('USCES_COL_SKU_CODE', 39 + $add_field_num);	//21
	define('USCES_COL_SKU_NAME', 40 + $add_field_num);	//22
	define('USCES_COL_SKU_CPRICE', 41 + $add_field_num);	//23
	define('USCES_COL_SKU_PRICE', 42 + $add_field_num);	//24
	define('USCES_COL_SKU_ZAIKONUM', 43 + $add_field_num);	//25
	define('USCES_COL_SKU_ZAIKO', 44 + $add_field_num);	//26
	define('USCES_COL_SKU_UNIT', 45 + $add_field_num);	//27
	define('USCES_COL_SKU_GPTEKIYO', 46 + $add_field_num);	//28
	...
	$err_num = 0;
	$line_num = 0;
	$min_field_num = apply_filters( 'dlseller_filter_uploadcsv_min_field_num', 47 + $add_field_num );
	$log = '';
	$pre_code = '';
	$sku_index = 0;
	$res = array();
		...
		foreach( $lines as $rows_num => $line ) {
				...
				switch( $key ) {
					...
					case USCES_COL_ITEM_CHARGING_TYPE:
						if( $data != 0 and $data != 1 ) {
							$mes = "No.".$line_num."\t".__('The value of the Charging Type is abnormal.', 'dlseller');
							$logtemp .= $mes.$yn;
							$mestemp .= $mes.$br.$yn;
						}
						break;
					...
					case USCES_COL_ITEM_DELIVERYMETHOD:
						if( $datas[USCES_COL_ITEM_DIVISION] == 'shipped' ) {//通常商品のとき
					...
					case USCES_COL_ITEM_CHARGINGDAY:
						if($datas[USCES_COL_ITEM_CHARGING_TYPE] == 1) {//継続課金のとき
							if( ( $data < 1 or $data > 28 ) and ( $data != 99 ) ) {
			...
			if( $pre_code != $item_code ) {
				...
				if( $mode == 'add' ) {
					...
					$meta_key_table = array( '_itemCode', '_itemName', '_itemRestriction', '_itemPointrate', '_itemGpNum1', '_itemGpDis1', '_itemGpNum2', '_itemGpDis2', '_itemGpNum3', '_itemGpDis3', '_itemShipping', '_itemDeliveryMethod', '_itemShippingCharge', '_itemIndividualSCharge', '_item_division', '_item_charging_type', '_item_frequency', '_item_chargingday', '_dlseller_interval', '_dlseller_validity', '_dlseller_file', '_dlseller_date', '_dlseller_version', '_dlseller_author', '_iopt_', '_isku_' );
				...
				if( isset($datas[USCES_COL_DLSELLER_INTERVAL])) {
					$valstr .= ( $usces->options['system']['csv_encode_type'] == 0 ) ? '('.$post_id.", '_dlseller_interval','".esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_INTERVAL], 'UTF-8', 'SJIS')))."')," : '('.$post_id.", '_dlseller_interval','".esc_sql( trim( $datas[USCES_COL_DLSELLER_INTERVAL] ))."'),";
				}
				if( $datas[USCES_COL_ITEM_DIVISION] == 'data') {
					if( isset($datas[USCES_COL_DLSELLER_VALIDITY])) {
						$valstr .= ( $usces->options['system']['csv_encode_type'] == 0 ) ? '('.$post_id.", '_dlseller_validity','".esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_VALIDITY], 'UTF-8', 'SJIS')))."')," : '('.$post_id.", '_dlseller_validity','".esc_sql( trim( $datas[USCES_COL_DLSELLER_VALIDITY] ))."'),";
					}
					if( isset($datas[USCES_COL_DLSELLER_FILE])) {
						$valstr .= ( $usces->options['system']['csv_encode_type'] == 0 ) ? '('.$post_id.", '_dlseller_file','".esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_FILE], 'UTF-8', 'SJIS')))."')," : '('.$post_id.", '_dlseller_file','".esc_sql( trim( $datas[USCES_COL_DLSELLER_FILE] ))."'),";
					}
					if( isset($datas[USCES_COL_DLSELLER_DATE])) {
						$dlseller_date = ltrim($datas[USCES_COL_DLSELLER_DATE], "'");
						$valstr .= ( $usces->options['system']['csv_encode_type'] == 0 ) ? '('.$post_id.", '_dlseller_date','".esc_sql(trim(mb_convert_encoding($dlseller_date, 'UTF-8', 'SJIS')))."')," : '('.$post_id.", '_dlseller_date','".esc_sql(trim( $dlseller_date ))."'),";
					}
					if( isset($datas[USCES_COL_DLSELLER_VERSION])) {
						$valstr .= ( $usces->options['system']['csv_encode_type'] == 0 ) ? '('.$post_id.", '_dlseller_version','".esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_VERSION], 'UTF-8', 'SJIS')))."')," : '('.$post_id.", '_dlseller_version','".esc_sql(trim($datas[USCES_COL_DLSELLER_VERSION]))."'),";
					}
					if( isset($datas[USCES_COL_DLSELLER_AUTHOR])) {
						$valstr .= ( $usces->options['system']['csv_encode_type'] == 0 ) ? '('.$post_id.", '_dlseller_author','".esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_AUTHOR], 'UTF-8', 'SJIS')))."')," : '('.$post_id.", '_dlseller_author','".esc_sql(trim($datas[USCES_COL_DLSELLER_AUTHOR]))."'),";
					}
				}
				$valstr = rtrim($valstr, ',');
	...
	//			wp_set_object_terms($post_id, (array)$tags, 'post_tag');
				...
				do_action( 'dlseller_action_uploadcsv_itemvalue', $post_id, $datas );
	...
	//					}
			...
			$skuvalue = apply_filters( 'dlseller_filter_uploadcsv_skuvalue', $skuvalue, $datas );
	...
	if( !empty($results) ) {
		...
		foreach( (array)$rows as $row ) {
			...
			foreach( $sku_meta as $sku_value ) {
				...
				$line_sku .= apply_filters( 'dlseller_filter_downloadcsv_skuvalue', '', $sku_value );
