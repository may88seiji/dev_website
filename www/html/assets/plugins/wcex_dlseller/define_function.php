...
//							$logtemp .= "No." . ($rows_num+1) . "\t" . __(sprintf('Option name of No.%s option is non-input.', ($i+1)), 'usces')."\r\n";
		...
		if( $pre_code != $datas[USCES_COL_ITEM_CODE] ){
			...
			$valstr .= '(' . $post_id . ", '_itemShippingCharge','" . esc_sql(trim(mb_convert_encoding($datas[USCES_COL_ITEM_SHIPPINGCHARGE], 'UTF-8', 'SJIS'))) . "'),";
			...
			if($datas[USCES_COL_ITEM_DIVISION] == 'data') {
				...
				if(isset($datas[USCES_COL_DLSELLER_FILE])) $valstr .= '(' . $post_id . ", '_dlseller_file','" . esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_FILE], 'UTF-8', 'SJIS'))) . "'),";
				if(isset($datas[USCES_COL_DLSELLER_DATE])) {
					$dlseller_date = ltrim($datas[USCES_COL_DLSELLER_DATE], "'");
					$valstr .= '(' . $post_id . ", '_dlseller_date','" . esc_sql(trim(mb_convert_encoding($dlseller_date, 'UTF-8', 'SJIS'))) . "'),";
				}
				if(isset($datas[USCES_COL_DLSELLER_VERSION])) $valstr .= '(' . $post_id . ", '_dlseller_version','" . esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_VERSION], 'UTF-8', 'SJIS'))) . "'),";
				if(isset($datas[USCES_COL_DLSELLER_AUTHOR])) $valstr .= '(' . $post_id . ", '_dlseller_author','" . esc_sql(trim(mb_convert_encoding($datas[USCES_COL_DLSELLER_AUTHOR], 'UTF-8', 'SJIS'))) . "'),";
			}
