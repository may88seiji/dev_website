...
*/	if( $ext == 'xls' ) {//TSV
	...
	$chk_con['order_date'] = ( isset($_REQUEST['check']['order_date']) ) ? 1 : 0;
	...
	$line = $table_h;
	$line_h = $tr_h;
	if( isset($_REQUEST['check']['order_id']) ) $line_h .= $th_h1.__('Order ID', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['deco_id']) ) $line_h .= $th_h.__('order number', 'usces').$th_f;
	if( isset($_REQUEST['check']['mem_id']) ) $line_h .= $th_h.__('membership number', 'usces').$th_f;
	if( isset($_REQUEST['check']['name']) ) $line_h .= $th_h.__('name', 'usces').$th_f;
	if( $applyform == 'JP' ) {
		if(isset($_REQUEST['check']['kana'])) $line_h .= $th_h.__('furigana', 'usces').$th_f;
	}
	if( isset($_REQUEST['check']['limitofcard'])) $line_h .= $th_h.__('Limit of Card(Month/Year)', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['price'])) $line_h .= $th_h.__('Total Amount', 'usces').'('.__(usces_crcode( 'return' ), 'usces').')'.$th_f;
	if( isset($_REQUEST['check']['acting'])) $line_h .= $th_h.__('Settlement Supplier', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['payment_name'])) $line_h .= $th_h.__('payment method', 'usces').$th_f;
	if( isset($_REQUEST['check']['orderdate'])) $line_h .= $th_h.__('Application Date', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['startdate'])) $line_h .= $th_h.__('First Withdrawal Date', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['contractedday'])) $line_h .= $th_h.__('Renewal Date', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['chargedday'])) $line_h .= $th_h.__('Next Withdrawal Date', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['status'])) $line_h .= $th_h.__('Status', 'dlseller').$th_f;
	if( isset($_REQUEST['check']['condition'])) $line_h .= $th_h.__('Condition', 'dlseller').$th_f;
	...
	foreach( (array)$rows as $data ) {
		$line_d = $tr_h;
		if( isset($_REQUEST['check']['order_id']) ) $line_d .= $td_h1.$data['order_id'].$td_f;
		if( isset($_REQUEST['check']['deco_id']) ) $line_d .= $td_h.$data['deco_id'].$td_f;
		if( isset($_REQUEST['check']['mem_id']) ) $line_d .= $td_h.$data['mem_id'].$td_f;
		if( isset($_REQUEST['check']['name']) ) $line_d .= $td_h.usces_entity_decode($data['name1'].' '.$data['name2'], $ext).$td_f;
		if( $applyform == 'JP' ) {
			if( isset($_REQUEST['check']['kana']) ) $line_d .= $td_h.usces_entity_decode($data['name3'].' '.$data['name4'], $ext).$td_f;
		}
		if( isset($_REQUEST['check']['limitofcard']) ) $line_d .= $td_h.$data['limitofcard'].$td_f;
		if( isset($_REQUEST['check']['price']) ) $line_d .= $td_h.$data['price'].$td_f;
		if( isset($_REQUEST['check']['acting']) ) {
		...
		if( isset($_REQUEST['check']['payment_name']) ) $line_d .= $td_h.$data['payment_name'].$td_f;
		if( isset($_REQUEST['check']['orderdate']) ) $line_d .= $td_h.$data['orderdate'].$td_f;
		if( isset($_REQUEST['check']['startdate']) ) $line_d .= $td_h.$data['startdate'].$td_f;
		if( isset($_REQUEST['check']['contractedday']) ) $line_d .= $td_h.$data['contractedday'].$td_f;
		if( isset($_REQUEST['check']['chargedday']) ) $line_d .= $td_h.$data['chargedday'].$td_f;
		if( isset($_REQUEST['check']['status']) ) {
			$continue_status = ( $data['status'] == 'continuation' ) ? __('continuation', 'dlseller') : __('cancellation', 'dlseller');
			$line_d .= $td_h.$continue_status.$td_f;
		}
		if( isset($_REQUEST['check']['condition']) ) $line_d .= $td_h.$data['condition'].$td_f;
