...
?>
	...
	<div class="postbox">
		<h3 class="hndle" id="ganbare"><span><?php _e('Ganbare Tencho','usces'); ?></span><?php echo $status; ?></h3>
		<div class="inside">
...
<?php
	...
	public function action_button() {
		echo '
		<input type="button" id="dl_ganbare_tencho_csv" class="searchbutton" value="' . __('Ganbare Tencho data output', 'usces') . '" />
		';
	}
	...
	public function outcsv_shipping() {
		...
		foreach( (array)$ids as $order_id ) {
			...
			if( 2 == $delivery['delivery_flag'] && !empty($data['mem_id']) && function_exists('msa_get_orderdestination') ){
			...
			for($i = 0; $i < count($cart); $i++) {
				...
				if( !empty($destination_info) ){
					...
					$ldata['送付先名字フリガナ'] = $delivery['name3'];
					$ldata['送付先名前フリガナ'] = $delivery['name4'];
