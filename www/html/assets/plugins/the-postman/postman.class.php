...
class Postman
{
  ...
  function mail()
  {
    ...
    foreach ($types as $type)
    {
      ...
      if (array_key_exists($type, $templates))
      {
        ...
        if ($args['is_spam'] === true) return false;
  ...
  function get($key = null)
  {
    ...
    // メタ情報(_ ではじまる)以外の、複数選択情報(チェックボックスなど)は文字列にして返す
    if(!preg_match('/^\_/', $key))
    {
      return !is_array($this->posts[$key]) ? stripslashes($this->posts[$key]) : stripslashes(join('／', $this->posts[$key]));
    }
    else
    {
